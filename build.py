...
MODULE_DESCRIPTIONS = {
    "backend": "Rust microservices orchestration",
    "frontend": "TypeScript/React legacy UI",
    "market": "Go market data and trading hooks",
    "frailbox": "C low-level utilities",
    "engine": "C++ trial engine",
    "compliance": "Java compliance auditor stub",
    "v2-market-stream": "Ruby market stream syntax check",
    "nfc-scanner": "Lua NFC scanner bytecode check",
    "openapi-tools": "Python/Lua OpenAPI tooling",
}
...
Diagnostic bundle:
  python3 build.py
    ...
    parser.add_argument(
        ...
        help="Show detailed build output (repeat for more verbosity)",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "-o", "--output-dir",
        help="Custom directory for diagnostic artifacts",
    )
    parser.add_argument(
        "--version", action="version", version=BUILD_VERSION,
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        help="List build targets with descriptions and exit",
    )
    ...
    if args.output_dir:
        ...
        DIAGNOSTIC_DIR = Path(args.output_dir)
        DIAGNOSTIC_DIR.mkdir(parents=True, exist_ok=True)

    if args.list_targets:
        print(f"  {color('Build targets:', Colors.BOLD)}")
        for m in MODULES:
            desc = MODULE_DESCRIPTIONS.get(m.name, m.language)
            print(f"    {color(m.name, Colors.CYAN)} - {desc}")
        return 0
